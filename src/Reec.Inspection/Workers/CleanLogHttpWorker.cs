...
{
    public class CleanLogHttpWorker : BackgroundService
    {
        private readonly IServiceScopeFactory _serviceScope;
        private readonly ReecExceptionOptions _options;
        private readonly IDateTimeService _dateTime;

        public CleanLogHttpWorker(IServiceScopeFactory serviceScope, ReecExceptionOptions options, IDateTimeService dateTime)
        {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            {
                CronExpression expression = CronExpression.Parse(_options.LogHttp.CronValue);
        ...
        private static async Task<string> Process(IServiceProvider service, ReecExceptionOptions option, IDateTimeService dateTime, CancellationToken cancellationToken)
        {
            ...
            while (count > 0)
            {
                var query = dbContext.LogHttps.AsNoTracking()
            ...
            return $"Proceso Completado: {nameof(CleanLogHttpWorker)} limpió {deletedTotal} filas (corte: {day})";
        }
